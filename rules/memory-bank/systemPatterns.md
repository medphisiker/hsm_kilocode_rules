...
- **ServiceManifest**: Описание автономного сервиса с поддержкой нескольких рантаймов (например, репозиторий с uv и Docker-образом собранным на базе этого же uv окружения может быть и uv-сервисом и docker-сервисом).
- **RegistryGroup**: Универсальное описание группы выбора (библиотеки или сервисы).